...
### ✨ 신규 기능
1. **콘텐츠 관리 시스템**
   - Soft Delete 구현 (is_active 필드)
   - 개별/일괄 활성화 제어
   - Vector DB 실시간 동기화

2. **UI/UX 개선**
   - 콘텐츠 정렬 기능 (채널, 제목, 길이, 등록일)
   - 필터링 기능 (채널별, 상태별)
   - 페이지네이션 버그 수정

3. **데이터 정합성**
   - 중복 데이터 정리 (42,619개 → 855개)
   - 레거시 컬렉션 삭제 (paragraphs, full_texts)
   - 통계 일관성 확보

### 🐛 버그 수정
- channel_id 파싱 오류 해결
- 비활성 항목 재활성화 불가 문제 해결
- 페이지네이션 파라미터 누락 수정
